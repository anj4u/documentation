...
.. note::
   ...
   Then, an option appears to register on the test server.
...
Go to :menuselection:`Accounting --> Customers --> Customers` and switch to the list view.
Select customers you want to verify and then click :menuselection:`Actions --> Verify Peppol`

.. image:: electronic_invoicing/peppol-verify-several.png
   :alt: verify registration of several contacts
...
You can see all ready invoices by using the :guilabel:`Peppol ready` filter or by clicking
:guilabel:`Peppol ready invoices` on the corresponding Sales journal from the Accounting dashboard.

.. image:: electronic_invoicing/peppol-ready-invoices.png
   :alt: Filter ready peppol invoices
...
before the cron runs by clicking on :guilabel:`Fetch Peppol invoice status` in the corresponding
Sales journal on the Accounting dashboard.

.. image:: electronic_invoicing/peppol-fetch-message-status.png
   :alt: peppol fetch bills
